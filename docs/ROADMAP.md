...
Pure-Python `grokkun_env` (`reset`/`step`) + Godot monkey parity + fixed-rule sanity (`flee`≫`idle`). Next: train player vs scripted spawner → train spawner → self-play. Embed spawner as in-game challenge later.
